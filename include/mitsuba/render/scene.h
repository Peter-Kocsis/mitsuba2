...
public:
    ...
    std::pair<DirectionSample3f, Spectrum>
    sample_emitter_object_direction(const Interaction3f &ref,
                                    const Int32 object_idx,
                                    const Point2f &sample,
                                    Mask active = true) const;
    ...
    void addEmitter(ref<Emitter> emitter){ m_children.push_back(emitter); m_emitters.push_back(emitter); }

    /// Pops the last emitter of the scene
    void popEmitter(){ m_children.pop_back(); m_emitters.pop_back(); }
